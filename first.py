...
for n in range(3, 66):
    ...
    data = data.assign(
        rnk_div=np.floor(data.groupby(['rnk_pbr'])['div_yield'].rank(method='first') / (data_size / 9 + 1 / 3)))
    data = data.assign(
        rnk_roe=np.floor(data.groupby(['rnk_pbr'])['roe'].rank(method='first',ascending=False) / (data_size / 9 + 1 / 3)))

    for i in range(0, 3):
        for j in range(0, 3):
            locals()['data_{}{}'.format(i, j)] = data[
                (data['rnk_pbr'] == i) & (data['rnk_div'] == j) & (data['rnk_roe'] == j)]
            locals()['data_{}{}'.format(i, j)] = pd.concat([locals()['data_{}{}'.format(i, j)], rtn[n - 3]], axis=1,
                                                           join='inner', ignore_index=True)  # 수익률 매칭
            locals()['data_name_{}{}'.format(i, j)][n - 3] = locals()['data_{}{}'.format(i, j)][0].reset_index(
                drop=True)
            locals()['data_{}{}'.format(i, j)] = locals()['data_{}{}'.format(i, j)][
                locals()['data_{}{}'.format(i, j)][12].notnull()]
            locals()['data_{}{}'.format(i, j)] = locals()['data_{}{}'.format(i, j)][
                locals()['data_{}{}'.format(i, j)][12] > 0]
...
for i in range(0, 3):
    for j in range(0, 3):
        return_final.iloc[i, j] = np.product(locals()['return_data_{}{}'.format(i, j)],
                                             axis=1)  # np.product로 하면 누적, np.average
...
for n in range(3, 65):
    for i in range(0, 3):
        for j in range(0, 3):
            len1 = len(
                locals()['data_name_{}{}'.format(i, j)][locals()['data_name_{}{}'.format(i, j)][n - 2].notnull()])
