...
for n in range(3, 66):
    ...
    med = np.median(data['roe'])
    data = data[data['roe'] > med]

    data_size = len(data)  # Row count
    data = data.assign(rnk_pbr=np.floor(data['pbr'].rank(method='first') / (data_size / 3 + 1 / 3)))
    data = data.assign(rnk_div=np.floor(data.groupby(['rnk_pbr'])['div_yield'].rank(method='first') / (data_size / 9 + 1 / 3)))
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            locals()['data_{}{}'.format(i, j)] = locals()['data_{}{}'.format(i, j)][locals()['data_{}{}'.format(i, j)][11].notnull()]
            locals()['data_{}{}'.format(i, j)] = locals()['data_{}{}'.format(i, j)][locals()['data_{}{}'.format(i, j)][11] > 0]
            locals()['return_data_{}{}'.format(i, j)].iloc[0, n - 3] = np.mean(locals()['data_{}{}'.format(i, j)][11])
